...
document.addEventListener('DOMContentLoaded', function() {
    ...
    async function searchGrants() {
        const searchTerm = document.getElementById('grantSearch').value;
        const category = document.getElementById('grantCategory').value;
        
        try {
            const formData = new FormData();
            formData.append('action', 'gi_get_grants_for_analysis');
            ...
            if (data.success) {
                displayGrantsList(data.data.grants);
            } else {
    ...
    // 助成金リストを表示
    function displayGrantsList(grants) {
        const container = document.getElementById('grantsListContainer');
        if (!container) return;
        
        const html = grants.map(grant => `
            <div class="grant-item bg-white rounded-xl p-6 border border-gray-200 hover:shadow-lg transition-all duration-300">
                <div class="flex items-start gap-4">
                    <input type="checkbox" 
                           id="grant_${grant.id}" 
                           class="grant-checkbox mt-2" 
                           value="${grant.id}"
                           data-grant='${JSON.stringify(grant)}'>
                    <label for="grant_${grant.id}" class="grant-label flex-1 cursor-pointer">
                        <div class="flex items-center gap-2 mb-2">
                            <div class="checkbox-indicator w-5 h-5 border-2 border-gray-300 rounded"></div>
                            <h4 class="font-bold text-gray-900">${grant.title}</h4>
                        </div>
                        <p class="text-gray-600 text-sm mb-3">${grant.description}</p>
                        <div class="grid grid-cols-2 gap-4 text-sm">
                            <div>
                                <span class="text-gray-500">助成額:</span>
                                <span class="font-semibold text-green-600">${grant.amount_display}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">締切:</span>
                                <span class="font-semibold text-red-600">${grant.deadline_display}</span>
                            </div>
                            <div>
                                <span class="text-gray-500">成功率:</span>
                                <span class="font-semibold text-blue-600">${grant.success_rate}%</span>
                            </div>
                            <div>
                                <span class="text-gray-500">難易度:</span>
                                <span class="font-semibold ${grant.difficulty === '易しい' ? 'text-green-600' : grant.difficulty === '普通' ? 'text-yellow-600' : 'text-red-600'}">${grant.difficulty}</span>
                            </div>
                        </div>
                    </label>
                </div>
    ...
    async function runAnalysis() {
        const selectedGrants = Array.from(document.querySelectorAll('.grant-checkbox:checked')).map(cb => cb.value);
        
        if (selectedGrants.length === 0) {
            alert('分析する助成金を選択してください');
            return;
        }
        
        try {
            const formData = new FormData();
            formData.append('action', 'gi_analyze_grants');
            formData.append('grant_ids', JSON.stringify(selectedGrants));
            formData.append('nonce', '<?php echo wp_create_nonce('gi_ajax_nonce'); ?>');
            ...
            if (data.success) {
                ...
                alert('分析に失敗しました: ' + data.data.message);
            }
        } catch (error) {
            console.error('Analysis error:', error);
            alert('分析処理でエラーが発生しました');
        }
    }
    ...
    function displayAnalysisResults(analysisData) {
        ...
        const statsHtml = `
            <div class="bg-white rounded-xl p-6 border border-gray-200 mb-6">
                <h3 class="text-xl font-bold text-gray-900 mb-4">分析統計</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="text-center">
                        <div class="text-2xl font-bold text-blue-600">${analysisData.statistics.total_grants}</div>
                        <div class="text-sm text-gray-600">選択助成金数</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-green-600">${(analysisData.statistics.total_amount / 10000).toFixed(0)}万円</div>
                        <div class="text-sm text-gray-600">総助成額</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-purple-600">${analysisData.statistics.average_success_rate.toFixed(1)}%</div>
                        <div class="text-sm text-gray-600">平均成功率</div>
                    </div>
                    <div class="text-center">
                        <div class="text-2xl font-bold text-orange-600">${analysisData.statistics.best_success_rate.toFixed(1)}%</div>
                        <div class="text-sm text-gray-600">最高成功率</div>
                    </div>
                </div>
            </div>
        `;
        ...
        const recommendationsHtml = `
            <div class="bg-white rounded-xl p-6 border border-gray-200 mb-6">
                <h3 class="text-xl font-bold text-gray-900 mb-4">推奨助成金 TOP3</h3>
                <div class="space-y-4">
                    ${analysisData.recommendations.slice(0, 3).map((grant, index) => `
                        <div class="flex items-center gap-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg">
                            <div class="w-8 h-8 bg-gradient-to-r from-blue-500 to-purple-600 text-white rounded-full flex items-center justify-center font-bold">
                                ${index + 1}
                            </div>
                            <div class="flex-1">
                                <h4 class="font-bold text-gray-900">${grant.title}</h4>
                                <div class="text-sm text-gray-600">
                                    助成額: ${grant.amount_display} | 成功率: ${grant.success_rate}% | スコア: ${grant.score.toFixed(2)}
                                </div>
                            </div>
                        </div>
                    `).join('')}
        ...
        container.innerHTML = statsHtml + recommendationsHtml;
        container.classList.remove('hidden');
        
        // チャートを描画
        if (analysisData.chart_data) {
            drawComparisonChart(analysisData.chart_data);
        }
    }

    // 比較チャートを描画
    function drawComparisonChart(chartData) {
        // Chart.jsを使用してチャートを描画
        // 実装は省略（Chart.jsライブラリが必要）
